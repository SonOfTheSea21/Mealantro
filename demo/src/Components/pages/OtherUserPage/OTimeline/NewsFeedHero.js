...
export default function NewsfeedHero() {
  ...
  const fetchFeed = async () => {
    try {
      const response = await axios.get(`http://localhost:5000/newsfeed/timeline/${oid}/${id}`);
      console.log(response);
      const formattedFeed = response.data.data.map(thread => ({
        poster_id: thread.poster_id,
        thread_id : thread.thread_id,
        profilePictureSrc: thread.user_photo,
            ...
            <div className="panel-body">
              ...
              {feed.map((thread, index) => (
                <Thread
                  ...
                  poster_id = {thread.poster_id}
                  thread_id = {thread.thread_id}
                  profilePictureSrc={thread.profilePictureSrc == null ? "https://st3.depositphotos.com/15648834/17930/v/600/depositphotos_179308454-stock-illustration-unknown-person-silhouette-glasses-profile.jpg" : thread.user_photo}
